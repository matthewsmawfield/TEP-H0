...
# mB is the RAW peak magnitude
# m_b_corr is the STANDARDIZED magnitude (corrected for stretch, color, and HOST MASS)
pan_calib['MB_raw_implied'] = pan_calib['mB'] - pan_calib['MU_SH0ES']

merged = pan_calib.merge(hosts, left_on='CID', right_on='pantheon_id', how='inner')
merged = merged.dropna(subset=['MB_raw_implied', 'sigma_inferred', 'z_cmb'])
...
corr = np.corrcoef(merged['sigma_sq'], merged['MB_raw_implied'])[0, 1]
print(f"Correlation between RAW MB and sigma^2: {corr:.3f}")
z = np.polyfit(merged['sigma_sq'], merged['MB_raw_implied'], 1)
