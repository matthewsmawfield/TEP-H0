...
class Step5M31Analysis:
    ...
    The Test:
    We compare the Period-Luminosity (P-L) relation of Cepheids across a continuous density
    gradient within M31:
    1.  **Inner Region (R < 5 kpc)**: Deep potential, high stellar density (Bulge-dominated).
        Temporal Shear is progressively suppressed; Cepheids approach standard-clock behaviour.
    2.  **Outer Region (R > 15 kpc)**: Shallow potential, low density (Disk-dominated).
        Temporal Shear is active; Cepheids exhibit period contraction.
    ...
    - We compute a continuous shear-suppression factor $S(\rho)$ for each Cepheid.
    ...
    def calculate_local_density(self, r_kpc):
        """
        Calculates local stellar mass density (M_sun/pc^3) and continuous
        shear-suppression factor S(rho) at radius r_kpc.
        ...
        rho_disk_kpc = (M_disk / (4 * np.pi * R_d**2 * z_d)) * np.exp(
            -r / R_d
        )  # M_sun/kpc^3
    ...
    def calculate_shear_suppression(self, rho_local):
        ...
        where rho_half ~ 0.5 M_sun/pc^3 and n ~ 2.
        S = 1: fully active shear (unscreened).
        S = 0: fully suppressed shear (deep screening).
        """
        rho_half = 0.5  # M_sun/pc^3; galactic half-suppression density
        n_steep = 2.0  # smooth transition steepness
        S = 1.0 / (1.0 + (rho_local / rho_half) ** n_steep)
        return S
    ...
    def _tag_phat_overlap(
        self, df: pd.DataFrame, phat_df: pd.DataFrame, radius_arcmin: float = 2.0
    ) -> pd.Series:
    ...
    def _compute_wesenheit_ri(
        self, df: pd.DataFrame, R: float, anchor: str = "i"
    ...
    def _matched_bootstrap_delta(
        ...
        if (
            match_on_error
            and (yerr_col is not None)
            and in_df[yerr_col].notna().any()
            and out_df[yerr_col].notna().any()
        ):
        ...
        for _ in range(n_boot):
            ...
            if e_edges is None:
                ...
                for j in range(len(p_edges) - 1):
                    g_in = in_df[
                        (in_df["logP"] >= p_edges[j]) & (in_df["logP"] < p_edges[j + 1])
                    ]
                    g_out = out_df[
                        (out_df["logP"] >= p_edges[j])
                        & (out_df["logP"] < p_edges[j + 1])
                    ]
                    k = min(len(g_in), len(g_out))
                    if k <= 0:
                        continue
                    in_idx.extend(
                        rng.choice(g_in.index.to_numpy(), size=k, replace=True).tolist()
                    )
                    out_idx.extend(
                        rng.choice(
                            g_out.index.to_numpy(), size=k, replace=True
                        ).tolist()
                    )
            else:
                ...
                for j in range(len(p_edges) - 1):
                    for k2 in range(len(e_edges) - 1):
                        g_in = in_df[
                            (in_df["logP"] >= p_edges[j])
                            & (in_df["logP"] < p_edges[j + 1])
                            & (in_df[yerr_col] >= e_edges[k2])
                            & (in_df[yerr_col] < e_edges[k2 + 1])
                        ]
                        g_out = out_df[
                            (out_df["logP"] >= p_edges[j])
                            & (out_df["logP"] < p_edges[j + 1])
                            & (out_df[yerr_col] >= e_edges[k2])
                            & (out_df[yerr_col] < e_edges[k2 + 1])
                        ]
            ...
            a_in, _ = self._weighted_intercept(
                sub_in["logP"].values, sub_in[y_col].values, slope=slope, yerr=None
            )
            a_out, _ = self._weighted_intercept(
                sub_out["logP"].values, sub_out[y_col].values, slope=slope, yerr=None
            )
    ...
    def _multivariate_pair_match(
        ...
        if len(in_df) < 10 or len(out_df) < 10:
            return (
                np.array([], dtype=int),
                np.array([], dtype=int),
                {"n_inner": int(len(in_df)), "n_outer": int(len(out_df))},
            )
    ...
    def _matched_delta_from_pairs(
        ...
        a_in, _ = self._weighted_intercept(
            in_sub["logP"].values, in_sub[y_col].values, slope=slope, yerr=None
        )
        a_out, _ = self._weighted_intercept(
            out_sub["logP"].values, out_sub[y_col].values, slope=slope, yerr=None
        )
        ...
        for _ in range(n_boot):
            ...
            a_in_b, _ = self._weighted_intercept(
                in_sub["logP"].values[sel],
                in_sub[y_col].values[sel],
                slope=slope,
                yerr=None,
            )
            a_out_b, _ = self._weighted_intercept(
                out_sub["logP"].values[sel],
                out_sub[y_col].values[sel],
                slope=slope,
                yerr=None,
            )
    ...
    def _ks_balance(
        self, inner: pd.DataFrame, outer: pd.DataFrame, cols: list[str]
    ) -> dict:
        out = {}
        for c in cols:
            x = (
                pd.to_numeric(inner.get(c, np.nan), errors="coerce")
                .astype(float)
                .values
            )
            y = (
                pd.to_numeric(outer.get(c, np.nan), errors="coerce")
                .astype(float)
                .values
            )
            okx = np.isfinite(x)
    ...
    def _type_consistent_match(
        ...
        if "Type" not in inner.columns or "Type" not in outer.columns:
            in_idx, out_idx, meta = self._multivariate_pair_match(
                inner, outer, feature_cols, caliper=caliper
            )
            meta["type_mode"] = "none"
        ...
        for t in types:
            ...
            if len(in_t) < 5 or len(out_t) < 5:
                continue
            ii, oo, m = self._multivariate_pair_match(
                in_t, out_t, feature_cols, caliper=caliper
            )
        ...
        return (
            np.asarray(in_idx_all, dtype=int),
            np.asarray(out_idx_all, dtype=int),
            meta,
        )

    def _ols_inner_effect(
        self,
        df: pd.DataFrame,
        y_col: str,
        x_cols: list[str],
        inner_col: str = "is_inner",
    ) -> dict:
    ...
    def _matched_bootstrap_delta_2d(
        self,
        inner: pd.DataFrame,
        outer: pd.DataFrame,
        y_col: str,
        ...
        if not (
            np.isfinite(x1_min)
            and np.isfinite(x1_max)
            and np.isfinite(x2_min)
            and np.isfinite(x2_max)
        ):
            return {
                "n_inner": int(len(in_df)),
                "n_outer": int(len(out_df)),
        ...
        for _ in range(n_boot):
            ...
            for i in range(len(x1_edges) - 1):
                for j in range(len(x2_edges) - 1):
                    g_in = in_df[
                        (in_df[col1] >= x1_edges[i])
                        & (in_df[col1] < x1_edges[i + 1])
                        & (in_df[col2] >= x2_edges[j])
                        & (in_df[col2] < x2_edges[j + 1])
                    ]
                    g_out = out_df[
                        (out_df[col1] >= x1_edges[i])
                        & (out_df[col1] < x1_edges[i + 1])
                        & (out_df[col2] >= x2_edges[j])
                        & (out_df[col2] < x2_edges[j + 1])
                    ]
                    k = min(len(g_in), len(g_out))
                    if k <= 0:
                        continue
    ...
    def run_analysis(self):
        ...
        if "Wmag" not in df.columns:
            print_status(
                "Wmag column not found in initial catalog. Attempting to fetch main table explicitly.",
                "WARNING",
            )
            try:
        ...
        z0 = 0.0
        df["Z_proxy"] = z0 + grad_dex_per_kpc * pd.to_numeric(
            df["R_kpc"], errors="coerce"
        )
        df["Z_proxy"] = (
            df["Z_proxy"] - pd.to_numeric(df["Z_proxy"], errors="coerce").mean()
        )

        print_status(
            f"Radial Range: {df['R_kpc'].min():.1f} - {df['R_kpc'].max():.1f} kpc",
            "INFO",
        )
        ...
        inner_df = inner_df[
            (inner_df["logP"] > p_min_log) & (inner_df["logP"] < p_max_log)
        ]
        outer_df = outer_df[
            (outer_df["logP"] > p_min_log) & (outer_df["logP"] < p_max_log)
        ]
        ...
        core_rho_mean = (
            float(inner_core_df["rho_local"].mean()) if len(inner_core_df) else np.nan
        )
        ...
        print_table(
            headers,
            rows,
            title=f"Screening Threshold Summary (rho_trans={rho_trans} M_sun/pc^3)",
        )
        ...
        a_in, err_in = self._weighted_intercept(
            inner_df["logP"].values,
            inner_df["Wmag"].values,
            slope=fixed_slope,
            yerr=None,
        )
        a_out, err_out = self._weighted_intercept(
            outer_df["logP"].values,
            outer_df["Wmag"].values,
            slope=fixed_slope,
            yerr=None,
        )
        ...
        print_status(
            f"Detected Offset (Inner - Outer): {delta_mag:+.4f} mag ({sigma_significance:.1f} sigma)",
            "RESULT",
        )
        ...
        print_status(
            "Running M31 robustness suite (matched subsamples and sensitivity tests)...",
            "SECTION",
        )
        ...
        if not phat_footprint.empty:
            ...
            if len(inner_phat) > 5 and len(outer_phat) > 5:
                ap, ep = self._weighted_intercept(
                    inner_phat["logP"].values,
                    inner_phat["Wmag"].values,
                    slope=fixed_slope,
                    yerr=None,
                )
                aop, eop = self._weighted_intercept(
                    outer_phat["logP"].values,
                    outer_phat["Wmag"].values,
                    slope=fixed_slope,
                    yerr=None,
                )
                dmp = ap - aop
                dep = (
                    float(np.sqrt(ep**2 + eop**2))
                    if np.isfinite(ep) and np.isfinite(eop)
                    else np.nan
                )
                ...
                print_status(
                    f"PHAT-restricted Offset: {dmp:+.4f} +/- {dep:.4f} mag", "RESULT"
                )
            else:
                print_status(
                    f"Insufficient PHAT overlap (Inner={len(inner_phat)}, Outer={len(outer_phat)}).",
                    "WARNING",
                )
                robustness_rows.append(
                    {
            ...
            print_status(
                "Skipping PHAT restriction (footprint download failed or empty).",
                "WARNING",
            )
        ...
        for s in [-3.1, -3.3, -3.5]:
            ai, ei = self._weighted_intercept(
                inner_df["logP"].values, inner_df["Wmag"].values, slope=s, yerr=None
            )
            ao, eo = self._weighted_intercept(
                outer_df["logP"].values, outer_df["Wmag"].values, slope=s, yerr=None
            )
            dm = ai - ao
            de = (
                float(np.sqrt(ei**2 + eo**2))
                if np.isfinite(ei) and np.isfinite(eo)
                else np.nan
            )
            robustness_rows.append(
                {
        ...
        if all(
            col in df.columns for col in ["rP1mag", "iP1mag", "e_rP1mag", "e_iP1mag"]
        ):
            ...
            if ok.sum() < 50:
                print_status(
                    "Insufficient r/i/color coverage for Wesenheit coefficient inference; skipping R scan.",
                    "WARNING",
                )
                R_grid = []
            else:
                ...
                def score(anchor, R0):
                    if not np.isfinite(R0):
                        return np.inf
                    w_hat, _ = self._compute_wesenheit_ri(
                        df.loc[ok], R=float(R0), anchor=anchor
                    )
                ...
                if not np.isfinite(R0):
                    print_status(
                        "Could not infer a stable Wesenheit coefficient R0; skipping R scan.",
                        "WARNING",
                    )
                    R_grid = []
                else:
                    print_status(
                        f"Inferred Wesenheit form: anchor='{anchor_choice}', R0={R0:.2f} (MAD score: i={score_i:.3f}, r={score_r:.3f})",
                        "INFO",
                    )
                    R_grid = np.linspace(max(R0 - 0.6, 0.0), R0 + 0.6, 13)

            for R in R_grid:
                w_ri, e_w_ri = self._compute_wesenheit_ri(
                    df, R=float(R), anchor=anchor_choice
                )
                df_tmp = df.copy()
                df_tmp["W_ri"] = w_ri
                ...
                inner_tmp = inner_tmp[
                    (inner_tmp["logP"] > p_min_log) & (inner_tmp["logP"] < p_max_log)
                ]
                outer_tmp = outer_tmp[
                    (outer_tmp["logP"] > p_min_log) & (outer_tmp["logP"] < p_max_log)
                ]

                ai, ei = self._weighted_intercept(
                    inner_tmp["logP"].values,
                    inner_tmp["W_ri"].values,
                    slope=fixed_slope,
                    yerr=None,
                )
                ao, eo = self._weighted_intercept(
                    outer_tmp["logP"].values,
                    outer_tmp["W_ri"].values,
                    slope=fixed_slope,
                    yerr=None,
                )
                dm = ai - ao
            ...
            print_status(
                "Pan-STARRS r/i photometry columns not available for alternative Wesenheit scan; skipping.",
                "WARNING",
            )
        ...
        mv_features = self._select_features_by_coverage(
            inner_mv, outer_mv, mv_candidates, min_frac=0.7
        )
        if "logP" not in mv_features:
            mv_features = ["logP"] + [c for c in mv_features if c != "logP"]

        in_idx, out_idx, mv_meta = self._type_consistent_match(
            inner_mv, outer_mv, mv_features, caliper=3.0
        )
        mv_res = self._matched_delta_from_pairs(
            inner_mv,
            outer_mv,
            in_idx,
            out_idx,
        ...
        ks = (
            self._ks_balance(inner_mv.loc[in_idx], outer_mv.loc[out_idx], mv_features)
            if len(in_idx) > 0
            else {}
        )
        ...
        robustness_rows.append(
            {
                ...
                "n_matched_mean": float(
                    mv_meta.get("n_matched_total", mv_meta.get("n_matched", np.nan))
                ),
                "n_matched_min": float(
                    mv_meta.get("n_matched_total", mv_meta.get("n_matched", np.nan))
                ),
        ...
        mv_features_s = self._select_features_by_coverage(
            inner_mv_s, outer_mv_s, mv_candidates, min_frac=0.7
        )
        if "logP" not in mv_features_s:
            mv_features_s = ["logP"] + [c for c in mv_features_s if c != "logP"]

        in_idx_s, out_idx_s, mv_meta_s = self._type_consistent_match(
            inner_mv_s, outer_mv_s, mv_features_s, caliper=3.0
        )
        mv_res_s = self._matched_delta_from_pairs(
            inner_mv_s,
            outer_mv_s,
            in_idx_s,
            out_idx_s,
        ...
        ks_s = (
            self._ks_balance(
                inner_mv_s.loc[in_idx_s], outer_mv_s.loc[out_idx_s], mv_features_s
            )
            if len(in_idx_s) > 0
            else {}
        )
        ...
        robustness_rows.append(
            {
                ...
                "slope_fixed": fixed_slope,
                "n_inner": int(
                    mv_meta_s.get("n_inner_total", mv_meta_s.get("n_inner", 0))
                ),
                "n_outer": int(
                    mv_meta_s.get("n_outer_total", mv_meta_s.get("n_outer", 0))
                ),
                "n_boot": int(mv_res_s.get("n_boot", 0)),
                "n_matched_mean": float(
                    mv_meta_s.get("n_matched_total", mv_meta_s.get("n_matched", np.nan))
                ),
                "n_matched_min": float(
                    mv_meta_s.get("n_matched_total", mv_meta_s.get("n_matched", np.nan))
                ),
        ...
        df_reg["pl_resid"] = pd.to_numeric(
            df_reg["Wmag"], errors="coerce"
        ) - fixed_slope * pd.to_numeric(df_reg["logP"], errors="coerce")
        ...
        ols = self._ols_inner_effect(
            df_reg, y_col="pl_resid", x_cols=x_ctrl, inner_col="is_inner"
        )
        robustness_rows.append(
            {
        ...
        print_status(
            f"Saved robustness summary to {self.output_robust_csv_path}", "SUCCESS"
        )
        ...
        robust_payload = {
            ...
            "density_threshold": {
                ...
                "inner_rho_mean_msun_pc3": float(rho_inner)
                if np.isfinite(rho_inner)
                else None,
                "outer_rho_mean_msun_pc3": float(rho_outer)
                if np.isfinite(rho_outer)
                else None,
                "inner_n": int(len(inner_df)),
                "inner_n_above_rho_trans": int(inner_above.sum()),
                "inner_frac_above_rho_trans": float(inner_frac_above)
                if np.isfinite(inner_frac_above)
                else None,
                "outer_n": int(len(outer_df)),
                "outer_n_above_rho_trans": int(outer_above.sum()),
                "outer_frac_above_rho_trans": float(outer_frac_above)
                if np.isfinite(outer_frac_above)
                else None,
                "inner_core_cut_kpc": float(core_cut),
                "inner_core_rho_mean_msun_pc3": float(core_rho_mean)
                if np.isfinite(core_rho_mean)
                else None,
                "inner_core_n": int(len(inner_core_df)),
                "inner_core_n_above_rho_trans": int(core_above.sum()),
                "inner_core_frac_above_rho_trans": float(core_frac_above)
                if np.isfinite(core_frac_above)
                else None,
            },
            ...
            "metallicity_required_gamma": {
                "dz_proxy": dz,
                "gamma_required_mag_per_dex": gamma_req,
            },
        ...
        print_status(
            f"Saved robustness summary JSON to {self.output_robust_json_path}",
            "SUCCESS",
        )
        ...
        plt.plot(
            x_range,
            fixed_slope_model(x_range, a_in),
            color=colors["accent"],
            linestyle="-",
            linewidth=3,
        ...
        plt.plot(
            x_range,
            fixed_slope_model(x_range, a_out),
            color=colors["blue"],
            linestyle="-",
            linewidth=3,
        ...
        try:
            ...
            fig = plt.figure(
                figsize=(14, 9)
            )  # Standard TEP size (consistent with others)
            ...
            for _ in range(1000):
                ...
                for j in range(len(p_edges) - 1):
                    g_in = inner_df[
                        (inner_df["logP"] >= p_edges[j])
                        & (inner_df["logP"] < p_edges[j + 1])
                    ]
                    g_out = outer_df[
                        (outer_df["logP"] >= p_edges[j])
                        & (outer_df["logP"] < p_edges[j + 1])
                    ]
                    k = min(len(g_in), len(g_out))
                    if k > 0:
                        in_idx.extend(
                            rng.choice(
                                g_in.index.to_numpy(), size=k, replace=True
                            ).tolist()
                        )
                        out_idx.extend(
                            rng.choice(
                ...
                if len(in_idx) > 10:
                    ...
                    a_in = np.mean(
                        sub_in["Wmag"].values - fixed_slope * sub_in["logP"].values
                    )
                    a_out = np.mean(
                        sub_out["Wmag"].values - fixed_slope * sub_out["logP"].values
                    )
            ...
            ax1.axvline(
                delta_mag,
                color=plot_colors["dark"],
                linestyle="--",
                linewidth=2,
                label="Baseline",
            )
            ax1.set_title(
                "Matched (logP) Bootstrap\nDistribution", fontsize=12, fontweight="bold"
            )
            ...
            if len(slope_rows) > 0:
                ax2.errorbar(
                    slope_rows["slope_fixed"].values,
                    slope_rows["delta_mean"].values,
                    yerr=slope_rows["delta_std"].values,
                    fmt="o-",
                    color=plot_colors["blue"],
                    ecolor=plot_colors["dark"],
                    capsize=4,
                    linewidth=2,
                    markersize=8,
                )
                ax2.axhline(
                    delta_mag, color=plot_colors["dark"], linestyle="--", linewidth=2
                )
            ...
            if len(rscan) > 0:
                ...
                ax3.errorbar(
                    rscan["R"].values,
                    rscan["delta_mean"].values + offset,
                    yerr=rscan["delta_std"].values,
                    fmt="o-",
                    color=plot_colors["accent"],
                    ecolor=plot_colors["dark"],
                    capsize=4,
                ...
                ax3.set_title(
                    "Extinction/Wesenheit Sensitivity", fontsize=12, fontweight="bold"
                )
